...
- One numeric outcome column per file; `margin_of_error` only on unemployment and homeownership

**Stata example:**

```stata
import delimited "cleaned_output_2001_2025_full_panel/real_gdp_per_capita_by_state.csv", clear
...
```r
library(readr)
gdp <- read_csv("cleaned_output_2001_2025_full_panel/real_gdp_per_capita_by_state.csv")
...
Both folders contain the same seven filenames; **use the `cleaned_output_*` folder that matches your panel choice.**

---

## Related files

- `DATA_SOURCES.md` — older notes comparing data source options
